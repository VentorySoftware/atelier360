...
-- Step 1: Create sequences and helper functions
...
-- Step 2: Drop all dependent policies and constraints
...
-- Step 3: Add new ID columns to all tables
...
-- Step 6: Replace old columns with new ones (drop and rename)
-- Works table
ALTER TABLE public.works DROP COLUMN id CASCADE;
...
-- Clients table
ALTER TABLE public.clients DROP COLUMN id CASCADE;
...
-- Appointments table
ALTER TABLE public.appointments DROP COLUMN id CASCADE;
...
-- Profiles table (most sensitive due to auth integration)
ALTER TABLE public.profiles DROP COLUMN id CASCADE;
ALTER TABLE public.profiles RENAME COLUMN new_id TO id;

-- Work categories table
ALTER TABLE public.work_categories DROP COLUMN id CASCADE;
...
ALTER TABLE public.contact_attempts DROP COLUMN id CASCADE;
...
-- Step 9: Recreate RLS policies for profiles with new string-based auth system
-- We need a custom function since auth.uid() returns UUID but now we use strings
...
BEGIN
    ...
    -- For new approach, we need to handle auth differently
    -- Since we can't easily map UUID to string without additional data
    -- We'll allow all authenticated users for now and handle this in application logic
    RETURN NULL;
END;
...
-- Step 10: Create triggers for auto-generating IDs on new inserts
